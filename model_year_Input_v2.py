...
def build_model(in_tech = "Input_year_Technology.xlsx", in_loc = "Input_year_Location.xlsx", in_timeseries = "Input_year_Timeseries.xlsx",\
                in_emisFules = "Input_year_Emissions_Fuels.xlsx", in_tariffs = "Input_year_Tariffs.xlsx",\
                    in_other = "Input_year_Other.xlsx", projectlife = 20, dt = 10, yeardivision = None, consHeat = "No", consCool = "No"):
    ...
    load_e = {}
    for y in year_i:
        load_e[y] = load_e_in * (1+load_e_change)**(y-1)
        
    if consHeat == "Yes":
        load_h_in = loadData.loc[:,"Load_h [kW]"].values
        load_h = {}
        for y in year_i:
            load_h[y] = load_h_in * (1+load_h_change)**(y-1)
    if consCool == "Yes":
        load_c_in = loadData.loc[:,"Load_c [kW]"].values
        load_c = {}
        for y in year_i:
            load_c[y] = load_c_in * (1+load_c_change)**(y-1)
    ...
    tariff_change = {}
    for y in year_i:
        kWdia_change[y] = (1+kWdia_change_rate)**(y-1)
        tar_feedin_change[y] = (1+tar_feedin_change_rate)**(y-1)
        tariff_change[y] = (1+tariff_change_rate)**(y-1)
        
    
    tariffs = pd.read_excel(in_tariffs, sheet_name = "Tariff_el")
    if tariffs.at[0,"Tariff [€/kW]"] != None:
        tar_kWdia_in = tariffs.at[0,"Tariff [€/kW]"]
        tar_kWdia = {}
        for y in year_i:
            tar_kWdia[y] = tar_kWdia_in * kWdia_change[y]
    else:
        for y in year_i:
            tar_kWdia[y] = 0
    ...
    if tariffs["Tariff_feed [€/kWh]"].isnull().values.any():
        tar_feedin_in = np.full_like(ts_i,tariffs.at[0,"Tariff_feed [€/kWh]"], dtype=np.double)
        tar_feedin = {}
        for y in year_i:
            tar_feedin[y] = tar_feedin_in * tar_feedin_change[y]
    ...
    if "Tariff_feed [€/kWh]" in tariffs_dt.columns:
        ...
        for y in year_i:
            tar_feedin[y] = tar_feedin_in * tar_feedin_change[y]
    tariff_in = tariffs_dt.loc[:,"Tariff [€/kWh]"].values
    tariff = {}
    for y in year_i:
        tariff[y] = tariff_in * tariff_change[y]
    ...
    irrad_tilt_flat = hp.irrad_tilt_v2(date_rng_hourly,irrad, loc, 180, loc["lat"], dt)
    
    for c in irrad_tilt_tilt_df.columns:
        for i in irrad_tilt_tilt_df.index:
            irrad_tilt_tilt_df.at[i,c] = hp.irrad_tilt_v2(date_rng_hourly,irrad, loc, i, c, dt)
            
    if "pv_wall" in ders_df.index.values:
        for c in irrad_tilt_wall_df.columns:
            for i in irrad_tilt_wall_df.index:
                irrad_tilt_wall_df.at[i,c] = hp.irrad_tilt_v2(date_rng_hourly,irrad, loc, i, 90, dt)
    
    irrad_tilt_flat_exist = {}
    for az in pv_ground_exist:
        irrad_tilt_flat_exist[az] = hp.irrad_tilt_v2(date_rng_hourly,irrad, loc, az, loc["lat"], dt)
    ...
    def revenue_rule(m, y):
        return m.revenue[y] == sum(m.gridExport[y, ts] * dth * tar_feedin[y][ts] for ts in ts_i) *\
                                yearMultiple
    ...
    def omFix_rule(m, y):
        return m.cost_OM_fix[y] == sum(m.c_c[d] * ders_df.at[d,"OM_fix"] for d in ders_c) + \
            ...
            m.maxgridImport[y] * 1.2 * tar_kWdia[y] * 365
    ...
    def omVar_rule(m, y):
        return m.cost_OM_var[y] == sum(m.elEner[d_e, y] * ders_df.at[d_e,"OM_var"] for d_e in d_elOut) + \
            ...
            sum(m.gridImport[y, ts] * tariff[y][ts] for ts in ts_i) * dth * yearMultiple
    ...
    if CO2_cost == True:
        def CO2Cost_rule(m, y):
            return m.cost_CO2[y] == sum(m.emiss_CO2_annual[tec, y] for tec in fueltec)\
                /1000 * CO2_price[y]
    ...
    if CO2_cap == True and fueltec.size != 0:
        def CO2cap_rule(m, y):
            return sum(m.emiss_CO2_annual[tec, y] for tec in fueltec) <= CO2_cap_value[y]
    ...
    def gridemiss_rule(m, y, ts):
        return m.emiss_CO2["grid", y, ts] == m.gridImport[y, ts] * emiss_fac["grid"]
    ...
    if c == 1:
        def cEner_rule(m, cSup, y):
            return m.cEner[cSup, y] == sum(m.ceat[cSup, cCons, y, ts] \
    ...
    if "pv_ground" in ders_df.index.values:
        ...
        def pv_groundGenRule(m, y, ts):
            return m.pv_groundGen[y, ts] == (m.pv_groundPanelArea * irrad_tilt_flat[ts])* pv_eff * pv_pr +\
    ...
    if "pv_roof" in ders_df.index.values:
        ...
        def pv_roofGenRule(m, y, ts):
            return m.pv_roofGen[y, ts] == sum((m.pv_roofPanelArea[az, tilt] + \
    ...
    if "heat_pump_air" in ders_df.index.values:
        # Heat generation by heat pump fed with electricity --> COP depends on outdoor temperature
        # Better model, 
        ...
        if c==0:
            ...
            def ashpHeat_rule(m,y,ts):
                return sum(m.heat["heat_pump_air", hCons, y, ts] for hCons in hIn) == \
                    sum(m.power[elSup, "heat_pump_air",y, ts] for elSup in elOut) * 3
            m.const_ashpHeat = Constraint(year_i, ts_i, rule = ashpHeat_rule)
        ...
        if c==1:
            #Binary variables for heating or cooling mode of heat pump
