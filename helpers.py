...
def irrad_tilt_v2(time, irrad, loc, pan_az, pan_tilt, resolution):
    ...
    for i in range(len(irrad.index)):        
        ...
        tilt_dir = irrad.at[i,"direct_n"] * np.maximum(0 , cos_tilt / \
                        np.sin(np.radians(90-sun_zenith)))
        tilt_diff = irrad.at[i,"diffuse_h"] * (1 + np.cos(np.radians(pan_tilt))) * 0.5
        tilt_refl = irrad.at[i,"global_h"] * loc['alb'] * (1 - np.cos(np.radians(pan_tilt))) * 0.5
